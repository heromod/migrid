...
def main(client_id, user_arguments_dict):
    ...
    if not stat:
        output_objects.append({'object_type': 'error_text', 'text'
                              : 'Error getting list of virtual orgs.'})
    ...
    for vgrid_name in list:
        ...
        if vgrid_name == default_vgrid:
            ...
            vgrid_obj['memberlink'] = {'object_type': 'link',
             'destination':'',
             'text': "<img src='/images/icons/information.png' title='Every user is member of the %s virtual org.' />" % default_vgrid }
        ...
        if vgrid_is_owner_or_member(vgrid_name, client_id, configuration):
            ...
            vgrid_obj['memberlink'] = {'object_type': 'link',
                ...
                'text': "<img src='/images/icons/cancel.png' title='Leave this Virtual Organization' />"}
...
'''
    ...
    output_objects.append({'object_type': 'header', 'text': 'Virtual Organizations'
                          })
    output_objects.append({'object_type': 'text', 'text'
                          : '''
Virtual Organizations (sometimes called VGrids on this server) share files
and resources. Members can access web pages, files and resources, owners can also edit pages, as well as add and remove members or resources.
'''                       })
    ...
    output_objects.append({'object_type': 'sectionheader', 'text'
                          : 'Virtual Organization Totals'})
    output_objects.append({'object_type': 'link', 'text'
                          : 'View a monitor page with all Virtual Organizations/resources you can access'
                          , 'destination'
                          : 'showvgridmonitor.py?vgrid_name=ALL'})

    output_objects.append({'object_type': 'sectionheader', 'text'
                          : 'Virtual Organization request'})
    output_objects.append({'object_type': 'link', 'text'
                          : 'Request ownership or membership of an existing Virtual Organization'
